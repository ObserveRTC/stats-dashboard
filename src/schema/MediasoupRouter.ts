 ...
 * NOTE — local widening.
 *
 * This file tracks the generated schema in observer-js. Two shapes are widened
 * locally, because the dashboard *reads* these samples rather than producing
 * them, and a reader must not drop what it does not recognise:
 *
 *   SampleHistoryItem      the generated type is `{type, timestamp}` only, but
 *                          some producers inline the changed value next to it
 *                          (a tuple on `iceselectedtuple-changed`, say). Kept as
 *                          an index signature so `HistoryEvent.payload` can pick
 *                          it up when present — never assumed to be there.
 *   the sample roots       widened so a field added upstream still reaches the
 *                          raw samples browser before this file is re-synced.
 *
 * Keep both when re-syncing from observer-js.
 */

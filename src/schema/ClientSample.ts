...
export type OutboundRtpStats = {
	...
	/**
	* Legacy (schema ≤3.4): the id of the MediaStreamTrack this RTP stream
	* carried. Dropped from the generated schema, kept here because stored
	* samples still have it and the track ↔ RTP join reads it.
	*/
	trackIdentifier?: string;
	/**
	* The timestamp for this stats object in DOMHighResTimeStamp format.
...
export type PeerConnectionSample = {
	...
	/**
	* Legacy (schema ≤3.4): extension stats reported per peer connection rather
	* than at the sample root. Dropped from the generated schema, kept here
	* because stored samples still have it.
	*/
